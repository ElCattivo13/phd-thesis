...
The second major drawback is that UML lacks expressive power to model context-dependent
...
%mentioned in Section~\ref{sec:adding-cont-concepts}.
...
\begin{definition}[Syntax of \LMLO]\label{def:syntax-lmlo}
  ...
  \end{itemize}
    %
  Finally, a \emph{Boolean \LMLO-knowledge base (\LMLO-BKB)} is a triple $\Bmf=(\B,\RO,\RM)$,
  where~\RO is an~\LO-RBox over~\Osig, \RM an \LM-RBox over~\Msig, and~\B is a Boolean m-axiom
...
\end{figure}
\footnotetext{A Hail Mary pass is a very long forward pass in American football, made in desperation
...
%  LocalWords:  ontologies workflow CROM CROMs
