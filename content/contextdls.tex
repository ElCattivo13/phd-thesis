...
\todo{here something more about constant domain assumption, references. Argument about
  ...
  \begin{tikzpicture}
    \node[rectangle,
          ...
          label={[align=left, anchor=south west]135:$\mathsf{NFL\_Team}$, $\mathsf{NFC}$, \\\textit{GreenBayPackers}}] (a) at (2.0,0){
            \begin{tikzpicture}
              \node[node,label={[align=left]270:\textit{MikeMcCarthy},\\ $\mathsf{Coach}$}] (mmc) at (0,3){};
              \node[node] (kid) at (2.5, 2.5){};
              \node[node,label={[align=left]270:\textit{AaronRodgers}, \\ $\mathsf{NFL\_Player}$}] (ar) at (0,0.5){};
              \node[node,label={[align=right]south:$\mathsf{Quarterback}$}] (qb) at (3,0.5){};
    ...
    \node[rectangle,
          ...
          label={[anchor=south]90:$\mathsf{Junior Power Pack Football Clinic}$}] (b) at (9.45,0){
            \begin{tikzpicture}
              \node[node,label={270:\textit{MikeMcCarthy}}] (mmc) at (0,3){};
              \node[node,label={0:$\mathsf{Kid}$}] (kid) at (2.5,2.5){};
              \node[node,label={[align=left]270:\textit{AaronRodgers},\\ $\mathsf{Coach}$}] (ar) at (0,0.5){};
              \node[node] (qb) at (3,0.5){};
              \draw[edge] (ar) to[bend left=20, swap] node{$\textsf{teaches\_HailMarys}\footnotemark$} (kid);
            \end{tikzpicture}
          };
...
To show that the constant domain assumption is no serious restriction, we can adapt the proof of
\cite{GKW+-03}. Let \Bmf be an \LMLO-BKB and let $\J_{v}$ denote a nested interpretation with
varying domains, i.e.\
...
fresh object concept name $E$ that expresses the existence of an element in an object domain and
obtain $\Bmf_{E}$ from \Bmf by replacing every occurrence of an object concept name $A$ in \Bmf with
$(A \sqcap E)$.
...
\begin{proof}
  For a complex object concept $C$ occurring in \Bmf, let $C_{E}$ be obtained from $C$ by replacing
  every occurrence of $A\in\OC$ in $C$ with $(A \sqcap E)$.

  Let $\J_{v} = (\Cbb,\cdot^{\J},(\Delta^{\I_{c}})_{c\in\Cbb},(\cdot^{\I_c})_{c\in\Cbb})$ be a model
  of \Bmf with varying domains. Then we construct the nested interpretation \JJ with a constant
  domain from $\J_{v}$ such that $\Delta^{\J}\coloneqq\bigcup_{c\in\Cbb}\Delta^{\I'_{c}}$ and
  $E^{\I_{c}} = \Delta^{\I_{c}}$.
\end{proof}
...
%  LocalWords:  Löwenheim Skolem Logics logics iff temporalized Quasimodel quasimodel BKB Klarman
