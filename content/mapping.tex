...
\item \emph{\rst participates in \ct} if $\rel(\rst)\subseteq\parts(\ct)$,
...
\begin{definition}[Constraint Set, Validity]
  ...
  \item All intra-relationship type constraints are respected, i.e.\ every function $\fsf \in
    ...
    \begin{align*}
      ...
      & \qquad\text{for all $c$ in $\ct^{\I}$ with $\rel(\rst)\subseteq\parts(\ct)$}.
    \end{align*}
  \end{enumerate}
...
\item $\NT\cup\RT \subseteq \OC$ since every natural type and every \rosirole type is an object concept, and
\item $\RST \subseteq \OR \setminus \ORR$ since every relationship type is an flexible object role.
\end{itemize}
Additionally, we have the flexible object role $\plays \in \OR \setminus \ORR$ to express the \plays-relation.
