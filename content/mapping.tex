...
published in~\cite{KBG-SLE15}. This variant is semantically equivalent to~\cite{KBG-SLE15} and we
...
that both \rosirole types are part of the same compartment type and that not the same \rosirole type
is the domain and the range.
...
\begin{example}\label{ex:bank-crom}
  ...
  the context of an transaction. Transactions in turn can have the function of a money transfer in
...
\begin{figure}
  \centering
  \includegraphics[width=\textwidth]{Bank-full-constraints-2}
...
constraints are formalized with so-called \emph{role groups}. These consist of a set of role types
...
properties and can be described, and that are organized in a relational structure.  This results in
...
application of contextualized knowledge, it is complexity-wise very costly, especially in the
...
%
For cardinality constraints, we also utilize number restrictions.
...
\item We formalize a general domain and range restriction for \plays. Only naturals or
