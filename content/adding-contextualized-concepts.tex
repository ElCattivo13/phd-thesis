...
are allowed.  With some assumptions, in \LMLOplus, however, Setting~(ii) that allows rigid concept names but no rigid
roles coincides with Setting~(i), since rigid concepts can be simulated. Hence, we obtain the
following result:
...
\begin{proof}
  ...
  Let $\Omf = (\Omc,\RO, \RM)$ be an \SHOISHOIplus-ontology. We first have to simulate a universal role $u$ on the meta level. We can assume
  w.l.o.g.\ that $u\in\MR$ does not occur in \Omf. We obtain $\Omf_{u}$ from \Omf by adding the
  following axioms to \Omf:
  \begin{itemize}
  \item $u^{-} \sqsubseteq u$,
    % \item $\trans{u}$,
