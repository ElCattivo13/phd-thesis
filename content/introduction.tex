...
The relational or context-dependent properties and behaviour of objects is
...
section, which at the same time are the lower bounds of the complexity for more expressive
contextualized DLs. For the complexity analysis we consider three different settings which result in
different complexity results. In the first setting we do not allow any rigid names at all.  In the
second setting, we assume the presence of rigid concepts but forbid any rigid roles whereas in the
...
assumptions of a CROM are translated.  We conclude that chapter with some thoughts on how to express
...
covers the analysis of the contextualised hypertableau algorithm. We finish that chapter with some
