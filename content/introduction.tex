...
subclass of $\mathsf{Customer}$.  This conflict can be resolved when we realise that
$\mathsf{Person}$ and $\mathsf{Company}$ are context-independent basic concepts, so-called
\emph{natural types} and $\mathsf{Customer}$ is a role a person or company can play in a
...
general does not have the syntactical means to express contextualised DL axioms. However, OWL gives
