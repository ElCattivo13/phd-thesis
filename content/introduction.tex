...
semantics of the ontology and define the contextual ontology in a way that all entities and
constraints appearing in the role-based model are mapped correctly. Therefore, it would be ideal to
have a mapping algorithm from role-based models into context DL knowledge bases.  In this thesis, we
...
equipped with a formal semantics. Otherwise, we cannot ensure that the intended meaning of some
model is correctly translated into the ontology. Furthermore, the modelling language needs the
expressive power to actually grasp all concepts needed in the role-based model.
%
...
Next, there is some freedom on how to express roles and role-playing in an ontology. Besides
arguments about the ontological nature of roles like identity or rigidity, practical reasons are
also important. Whether some constraints of a role-based model can be expressed in an ontology
...
implementation of the mapping. We based our implementation on the reference implementation for CROM\todo{cite}, which in turn can be used by FRaMED\todo{cite}, a graphical editor allowing the
...
In Chapter~\ref{cha:preliminiaries} we present in the first section the basic definitions of
...
%
In the second section, we give an ontological overview of the notion of a role and define properties
that are important to distinguish roles from other predicates.

In Chapter~\ref{cha:context-dls} we introduce the contextualized description logic \LMLO. We again
start with the definition of the syntax and semantics. For the complexity analysis of the
consistency problem we first look into the upper bounds. We consider the case of the lightweight
description logic \EL in a separate section, which at the same time are the lower bounds of the
complexity for more expressive contextualized DLs. For the complexity analysis we consider three
different settings which result in different complexity results. In the first setting we do not
allow any rigid names at all. 
% This is complexity-wise the simplest setting and we can show that the
% complexity of the consistency problem can be retained. 
In the second setting, we assume the presence
of rigid concepts but forbid any rigid roles whereas in the last and complexity-wise hardest setting we allow both
rigid concepts and rigid names. 
%
...
In
Chapter~\ref{cha:mapping} we present the mapping from role-based models into \LMLO-ontologies.
Therefore we introduce a syntactical variant of the Compartment Role Object Model and explain in
detail how the different assumptions of a CROM are translated. 
We conclude that chapter with some thoughts on how to express constraints for role-based models that
go beyond CROM.

Finally, in Chapter~\ref{cha:jconht} we present our reasoner for \LMLO.\todo{first finish Ch 5}
