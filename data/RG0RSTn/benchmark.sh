...
if [ ! -f "models.csv" ] ; then
	echo "Number;Breadth;Depth;RoleGroups;Concepts" > "models.csv"	
fi
...
for i in {5,10,15,20,25,30,35,40} ; do
	...
	for j in {1..100} ; do
		if [ ! -f "$i/test$i-$j.crom" ] ; then
			...
			/usr/bin/time -o "$i/benchmark.csv" -a -f "%e;%U;%P;%S;%x" $JAVA -jar org.rosi.crom.benchmark.jar -s$j -n$i -b$b -d$d -rg$rg "$i/test$i-$j.crom" ;
		fi		
	done
        c=`$JAVA -jar org.rosi.crom.benchmark.jar -n$i -b$b -d$d -rg$rg "tmp.crom"`
	echo "$i;$b;$d;$rg;$c" >> "models.csv"	
