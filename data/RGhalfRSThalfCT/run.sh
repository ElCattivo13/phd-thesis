...
for i in {5,10,15,20,25} ; do #,10
	...
	for j in {1..100} ; do
		if [ ! -f "$i/test$i-$j.crom" ] ; then
			...
			/usr/bin/time -o "$i/benchmark.csv" -a -f "%e;%U;%P;%S;%x" $JAVA -jar org.rosi.crom.benchmark.jar -s$j -n$i -b$b -d$d -rg$rg -rst$rst -fct "$i/test$i-$j.crom" ;
		fi		
	done
        c=`$JAVA -jar org.rosi.crom.benchmark.jar -n$i -b$b -d$d -rg$rg -rst$rst -fct "tmp.crom"`
